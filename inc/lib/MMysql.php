...
class MMysql {
	...
	protected $_dbName = DB_NAME; //数据库名
	...
	public static $LastID = ';SELECT LAST_INSERT_ID();';
	...
	public function insert($tbName,array $data){
		...
		$sql="insert {$tbName}({$keys}) VALUES ({$values})";
		$res=mysql_query($sql);
		if($res){
	 ...
	 * @param array $data
	 */
	public function getLastId($tbName,array $data){
